...
export const AuthService = {
  // ... (login, register, verifyEmail, etc. mantenha como estão)
  ...
  // ... (checkServerStatus, getCurrentUser, etc. mantenha como estão)
  
  // Certifique-se de manter as outras funções abaixo!
  login: async (email: string, role: Role, password?: string): Promise<User | null> => {
    // ... (código existente)
    const response = await apiRequest<{ token: string, user: User }>('/auth/login', {
      method: 'POST',
